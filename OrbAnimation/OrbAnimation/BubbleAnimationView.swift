...
struct BubbleAnimationView: View {
    ...
    func createBubbles() {
        for _ in 0..<50 {
            ...
            let bubble = Bubble(size: size, positionX: positionX, yOffset: CGFloat(positionY), speed: speed, opacity: opacity)
    ...
    func animateBubble(at index: Int) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
            withAnimation(.linear(duration: bubbles[index].speed).repeatForever(autoreverses: false)) {
